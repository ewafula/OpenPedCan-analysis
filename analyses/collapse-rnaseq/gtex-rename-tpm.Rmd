...
library("tidyverse")
library("VennDiagram")
# merge and collapse files
gtex_read_tpm <- readRDS("../../data/v4/gtex-gene-expression-rsem-tpm-collapsed.polya.rds")
target_read_tpm <- readRDS("../../data/v4/target-gene-expression-rsem-tpm-collapsed.rds")
tcga_read_tpm <- readRDS("../../data/v4/tcga-gene-expression-rsem-tpm-collapsed.rds")

read_tpm <- cbind(gtex_read_tpm,
                  tcga_read_tpm,
                  target_read_tpm,
...
```{r}
# get col index to rename  
rename_cols_index_target <- which(colnames(read_tpm) %in% histology_target$sample_id.y[!is.na(histology_target$sample_id.y)])
# get colnames for samples that don't match histology
...
rename_cols_barcode_target <-unlist(lapply (
  colnames(read_tpm)[rename_cols_index_target] ,
  function(x) 
...
```{r}
# get col index to rename  
rename_cols_index_tcga <- which(colnames(read_tpm) %in% histology_tcga$sample_id.y[!is.na(histology_tcga$sample_id.y)])
# get colnames for samples that don't match histology
remove_cols_tcga <-setdiff(colnames(read_tpm),
                          histology_tcga$sample_id.y)


# get barcode to replace colnames  
rename_cols_barcode_tcga <-unlist(lapply (
  colnames(read_tpm)[rename_cols_index_tcga] ,
  function(x) 
...
read_tpm %>%
  ...
  saveRDS("../../data/v5/gtex_target_tcga-gene-expression-rsem-tpm-collapsed.polya.rds")
