...
In this notebook we will subset `gtex_target_tcga-gene-counts-rsem-expected_count-collapsed.rds` from v4 release to only samples that have Kids_First_Biospecimen_ID ( GTEX Barcode , TCGA aliquot_id, TARGET barcode) assigned in `histologies.tsv`
...
#colnames iin rsem
colnames_rsem <- read_tsv("input/rownames_rsem.tsv") %>% deframe()
```
...
rename_cols_index_gtex <- which(colnames_rsem %in% histology_gtex$sample_id.y[!is.na(histology_gtex$sample_id.y)])
# get colnames for samples that don't match histology
c <-setdiff(mapping_KR$sample_id,
                           histology_gtex$sample_id.y)


# get barcode to replace colnames  
rename_cols_barcode_gtex <-unlist(lapply (
  colnames_rsem[rename_cols_index_gtex] ,
  function(x) 
...
```{r}
# get col index to rename  
rename_cols_index_target <- which(colnames_rsem %in% histology_target$sample_id.y[!is.na(histology_target$sample_id.y)])
# get colnames for samples that don't match histology
...
rename_cols_barcode_target <-unlist(lapply (
  colnames_rsem[rename_cols_index_target] ,
  function(x) 
...
```{r}
# get col index to rename  
rename_cols_index_tcga <- which(colnames_rsem %in% histology_tcga$sample_id.y[!is.na(histology_tcga$sample_id.y)])
# get colnames for samples that don't match histology
remove_cols_tcga <-setdiff(colnames_rsem,
                          histology_tcga$sample_id.y)


# get barcode to replace colnames  
rename_cols_barcode_tcga <-unlist(lapply (
  colnames_rsem[rename_cols_index_tcga] ,
  function(x) 
